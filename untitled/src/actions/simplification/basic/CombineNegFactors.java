...
public class CombineNegFactors {
    public Example createEx(Expression e){
        List<FactorGroup> list=e.factorGroups().filter(
                p->p.factors.subList(1).anyTrue(q->q instanceof Int i&&i.i<0));
        ...
        List<FactorGroup> newGroups=new List<>();
        for(int i=0;i<list.size();i++){
            ...
            int n=group.factors.countMatches(q->q instanceof Int j&&j.i<0);
            ...
            step22.innerCases=List.of(new InnerCase(List.of(case1,case2),n%2));
            FactorGroup newGroup=new FactorGroup(group.factors.fill(p->p instanceof Int j&&j.i<0?Int.of(-j.i):p));
            newGroups.push(newGroup);
            step22.strAfter="$$"+exp+"="+newGroup.toExpression().setColor(Color.BLUE).toString()+"$$";
            step.substeps=List.of(step21,step22);
        }
